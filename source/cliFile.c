...
CLIFile* CLIFile_Create(uint8_t* pData, uint32_t pLength, const char* pFilename)
{
	...
	if (dosHeader->Signature != UINT16_FROM_LE(CLIFILE__PE__DOS_Signature))
		return NULL;

	uint32_t offset = UINT32_FROM_LE(dosHeader->NextHeaderOffset);

	// -----------------------------------------------------------------------------------------------------
	//
	// -----------------------------------------------------------------------------------------------------
	PENTHeader* ntHeader = (PENTHeader*)(pData + offset);
	if (ntHeader->Signature != UINT32_FROM_LE(CLIFILE__PE__NT_Signature))
		return NULL;
	if(ntHeader->Machine != UINT16_FROM_LE(CLIFILE__Machine))
	    return NULL;

	SWAP32(ntHeader->Timestamp);
	SWAP32(ntHeader->PointerToSymbolTable);
	SWAP32(ntHeader->NumberOfSymbols);
	SWAP16(ntHeader->Machine);
	SWAP16(ntHeader->NumberOfSections);
	SWAP16(ntHeader->SizeOfOptionalHeader);
	SWAP16(ntHeader->Characteristics);


	// -----------------------------------------------------------------------------------------------------
	//
	// -----------------------------------------------------------------------------------------------------
	PEOptionalHeader* peOptionalHeader = (PEOptionalHeader*)(pData + offset + sizeof(PENTHeader));
	SWAP32(peOptionalHeader->SizeOfCode);
	SWAP32(peOptionalHeader->SizeOfUninitializedData);
	SWAP32(peOptionalHeader->AddressOfEntryPoint);
	SWAP32(peOptionalHeader->BaseOfCode);
	SWAP32(peOptionalHeader->BaseOfData);
	SWAP16(peOptionalHeader->Magic);

	if (peOptionalHeader->Magic == 0x10B)
	{
		offset += sizeof(PENTHeader) + sizeof(PEOptionalHeader);
		SWAP32(peOptionalHeader->SizeOfInitializedData);
		SWAP32(peOptionalHeader->ImageBase); 	/* must be 0x400000 */
		SWAP32(peOptionalHeader->SizeOfStackReserve);
		SWAP32(peOptionalHeader->SizeOfStackReserve);
		SWAP32(peOptionalHeader->SizeOfHeapReserve);
		SWAP32(peOptionalHeader->SizeOfHeapCommit);
	}
	else if (peOptionalHeader->Magic == 0x20B) /* PE32+ file format */
	{
		Panic("PE32+ file format --> not implemented!");
	}

	SWAP32(peOptionalHeader->SectionAlignment);       
	SWAP32(peOptionalHeader->FileAlignment);      
	SWAP16(peOptionalHeader->MajorOperatingSystemVersion);       
	SWAP16(peOptionalHeader->MinorOperatingSystemVersion);         
	SWAP16(peOptionalHeader->MajorImageVersion);
	SWAP16(peOptionalHeader->MajorImageVersion);
	SWAP16(peOptionalHeader->MajorSubsystemVersion);
	SWAP16(peOptionalHeader->MinorSubsystemVersion);
	SWAP32(peOptionalHeader->Reserved);
	SWAP32(peOptionalHeader->SizeOfImage);
	SWAP32(peOptionalHeader->SizeOfHeaders);
	SWAP32(peOptionalHeader->Checksum);
	SWAP16(peOptionalHeader->Subsystem);
	SWAP16(peOptionalHeader->DllCharacteristics);
	...
	//
	// -----------------------------------------------------------------------------------------------------
	PESectionHeader* peSectionHeaders = (PESectionHeader*)(pData + offset);

#if BIGENDIAN
	PESectionHeader* tmpSection = peSectionHeaders;
	for (int i = 0; i < ntHeader->NumberOfSections; i++)
	{
		SWAP32(tmpSection->Misc.VirtualSize);
		SWAP32(tmpSection->VirtualAddress);
		SWAP32(tmpSection->SizeOfRawData);
		SWAP32(tmpSection->PointerToRawData);
		SWAP32(tmpSection->PointerToRelocations);
		SWAP32(tmpSection->PointerToLineNumbers);
		SWAP16(tmpSection->NumberOfRelocations);
		SWAP16(tmpSection->NumberOfLineNumbers);
		SWAP32(tmpSection->Characteristics);
		++tmpSection;
	}
#endif
...
#if BIGENDIAN
	#define SWAPPDE(x) do { (x).VirtualAddress = UINT32_FROM_LE ((x).VirtualAddress); (x).Size = UINT32_FROM_LE ((x).Size);} while (0)
	...
	SWAPPDE(cliHeader->Resources);
	SWAPPDE(cliHeader->StrongNameSignature);
	SWAPPDE(cliHeader->CodeManagerTable);
	SWAPPDE(cliHeader->VTableFixups);
	SWAPPDE(cliHeader->ExportAddressTableJumps);
	...
	if (*(uint32_t*)(cliMetadataHeader + CLIFILE__Metadata__HeaderOffset_Signature) != UINT32_FROM_LE(CLIFILE__Metadata__Signature))
		return NULL;
	...
	uint32_t versionLength = READ32(cliMetadataHeader + CLIFILE__Metadata__HeaderOffset_VersionLength);
	uint16_t streamCount = READ16(cliMetadataHeader + CLIFILE__Metadata__HeaderOffset_Version + versionLength + CLIFILE__Metadata__HeaderVersionOffset_StreamCount);
	...
	for (uint16_t streamIndex = 0; streamIndex < streamCount; ++streamIndex)
	{
		...
		if (!strcmp(streamName, "#~"))
			file->Tables = streamData;
		else if (!strcmp(streamName, "#Strings"))
			file->StringsHeap = streamData;
		else if (!strcmp(streamName, "#US"))
			file->UserStringsHeap = streamData;
		else if (!strcmp(streamName, "#Blob"))
			file->BlobsHeap = streamData;
		else if (!strcmp(streamName, "#GUID"))
			file->GUIDsHeap = streamData;
	...
	while (gCLIFile_MetadataHandler_Table[tableCount].Initialize)
		++tableCount;

	for (uint32_t tableIndex = 0; tableIndex < tableCount; ++tableIndex) 
		tableData = gCLIFile_MetadataHandler_Table[tableIndex].Initialize(file, tableData);
	for (uint32_t tableIndex = 0; tableIndex < tableCount; ++tableIndex) 
		tableData = gCLIFile_MetadataHandler_Table[tableIndex].Load(file, tableData);
...
void CLIFile_Destroy(CLIFile* pFile)
{
	...
	while (gCLIFile_MetadataHandler_Table[tableCount].Initialize)
		++tableCount;
	for (uint32_t tableIndex = 0; tableIndex < tableCount; ++tableIndex)
		gCLIFile_MetadataHandler_Table[tableIndex].Cleanup(pFile);
	free(pFile->Filename);
	free(pFile);
}
...
PESectionHeader* CLIFile_GetSection(PESectionHeader* pSections, uint16_t pSectionCount, uint32_t pVirtualAddress)
{
	...
	{
		if (pVirtualAddress >= section->VirtualAddress && pVirtualAddress < (section->VirtualAddress + section->Misc.VirtualSize)) 
			return section;
		++section;
		--sectionCount;
	}
...
void CLIFile_Link(CLIFile* pFile)
{
	...
	for (uint32_t index = 1; index <= pFile->MethodDefinitionCount; ++index)
	{
		...
		//if ((pFile->MethodDefinitions[index].ImplFlags & MethodImplAttributes_InternalCall) != 0) 
		//    pFile->MethodDefinitions[index].InternalCall = ILDecomposition_ResolveInternalCall(&pFile->MethodDefinitions[index], pFile);
	}
